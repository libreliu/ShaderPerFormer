...
# copied from spirv.core.grammar.json
...
SpvInstrClasses = set(SpvInstrClassesOrdered)

# OpsToClass[opname] = Class
OpsToClass = {}

# OpsClass[className] = set(Opname)
OpsClass = {clsName: set() for clsName in SpvInstrClasses}

SpvGrammarPath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "./spirv.core.grammar.json")

# initialize ops
def initOps(opsToClass: dict, opsClass: dict):
  with open(SpvGrammarPath, "r") as f:
    spvGrammarJson = json.load(f)
    
    for instr in spvGrammarJson["instructions"]:
      if instr["class"] not in SpvInstrClasses:
        raise Exception(f"Instruction {instr['opname']} (Class: {instr['class']}) doesn't belong to any")
      instrClass = instr["class"]
      instrName = instr["opname"]
      opsToClass[instrName] = instrClass
      opsClass[instrClass].add(instrName)

initOps(OpsToClass, OpsClass)

class SpvTextContext:
  def __init__(self, instrCallback):
    """instrCallback(opcode, result_id or None, [args])"""
    self.instrCallback = instrCallback

  def parse(self, spvText):
    ss = io.StringIO(spvText)
    for line in ss:
      # silly method; only works for spv-dis
      line = line.strip()
      if len(line) == 0:
        continue
      ...
      line_ops = line.split()
      if line_ops[0][0] == "%":
        self.instrCallback(line_ops[2], line_ops[0], line_ops[3:])
      else:
        self.instrCallback(line_ops[0], None, line_ops[1:])
...
%mainImage_vf4_vf2_ = OpFunction %void None %23
 ...
 %resetTimer = OpVariable %_ptr_Function_float Function
  ...
  def callback(opcode, result, ops):
    num_instr_by_class[OpsToClass[opcode]] += 1

  ctx = SpvTextContext(callback)
  ctx.parse(exampleSpv)

  print(num_instr_by_class)