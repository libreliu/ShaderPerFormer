...
class RobertaFastConfig(RobertaConfig):
    model_type = "roberta"

    def __init__(self, attention_type=None, **kwargs):
...
class RobertaFastPreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = RobertaFastConfig
    ...
    def _set_gradient_checkpointing(self, module, value=False):
        if isinstance(module, RobertaFastEncoder):
...
class RobertaFastSelfAttention(nn.Module):
    def __init__(self, config: 'RobertaFastConfig', position_embedding_type=None):
        super().__init__()
...
class RobertaFastAttention(nn.Module):
    def __init__(self, config, position_embedding_type=None):
        super().__init__()
        self.self = RobertaFastSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = RobertaSelfOutput(config)
        self.pruned_heads = set()

    # What's heads pruning? See
    # https://github.com/huggingface/transformers/issues/850
    # https://arxiv.org/abs/1905.10650
    def prune_heads(self, heads):
        raise NotImplementedError("Pruning heads are not implemented yet")
...
class RobertaFastLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = RobertaFastAttention(config)
        self.is_decoder = config.is_decoder
        self.add_cross_attention = config.add_cross_attention
        if self.add_cross_attention:
            if not self.is_decoder:
                raise ValueError(f"{self} should be used as a decoder model if cross attention is added")
            self.crossattention = RobertaFastAttention(config, position_embedding_type="absolute")
...
class RobertaFastEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaFastLayer(config) for _ in range(config.num_hidden_layers)])
...
class RobertaFastModel(RobertaFastPreTrainedModel):
    """
    ...
    def __init__(self, config, add_pooling_layer=True):
        ...
        self.encoder = RobertaFastEncoder(config)
...
class RobertaForSequenceRegressionMAPE(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
class RobertaForSequenceClassificationFast(RobertaFastPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaFastModel(config, add_pooling_layer=False)
        self.classifier = RobertaClassificationHead(config)
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.config.problem_type == "regression" or self.config.problem_type == "regression-mse":
                loss_fct = MSELoss()
                if self.num_labels == 1:
                    loss = loss_fct(logits.squeeze(), labels.squeeze())
                else:
                    loss = loss_fct(logits, labels)
            if self.config.problem_type == "regression-mape":
                loss_fct = MAPELoss()
                if self.num_labels == 1:
                    loss = loss_fct(logits.squeeze(), labels.squeeze())
                else:
...
class RobertaTracedMultFast(RobertaFastPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaFastModel(config, add_pooling_layer=False)
        self.inst_weight_outputs = nn.Linear(config.hidden_size, 1)
    ...
    def forward(
        ...
        # shape (bsz, seq_len, 1)
        pooled_output = self.inst_weight_outputs(sequence_output)

        # shape (bsz, seq_len)
        pooled_output = torch.squeeze(pooled_output, 2)

        # shape (bsz, seq_len)
        trace_labels = trace_labels.to(pooled_output.device, dtype=torch.float32)

        # shape (bsz, )
        # pooled_output = torch.einsum("ij,ij->i", pooled_output, trace_labels)
        pooled_output = torch.sum(pooled_output * trace_labels, dim=(1,))

        loss = None
        if labels is not None:
            # move labels to correct device to enable model parallelism
            # shape (bsz, )
            labels = labels.to(pooled_output.device)

            if self.config.problem_type == "regression-mse":
                loss_fct = MSELoss()
                loss = loss_fct(pooled_output, labels)
        ...
        if not return_dict:
            output = (pooled_output,) # + outputs[2:]
            return ((loss,) + output) if loss is not None else output
...
# With one extra Scale + bias transform
class RobertaTracedMultWithAffineTransformFast(RobertaFastPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaFastModel(config, add_pooling_layer=False)
        self.inst_weight_outputs = nn.Linear(config.hidden_size, 1)
        self.inst_post_affine = nn.Linear(1, 1, bias=True)
    ...
    def forward(
        ...
        # shape (bsz, 1)
        # Or we'll get RuntimeError: mat1 and mat2 shapes cannot be multiplied (1x2 and 1x1)
        pooled_output = pooled_output.unsqueeze(-1)
        
        # shape (bsz, 1)
        pooled_output = self.inst_post_affine(pooled_output)
...
class RobertaTracedEmbedMultFast(RobertaFastPreTrainedModel):
    pass
...
class RobertaRegressionModelFast(RobertaFastPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.roberta = RobertaFastModel(config, add_pooling_layer=False)
        self.regression_head = RobertaRegressionHead(config)
    ...
    def forward(
        ...
        # shape (bsz, )
        regression_output = torch.squeeze(regression_output, 1)

        loss = None
        if labels is not None:
            # move labels to correct device to enable model parallelism
            # shape (bsz, )
            labels = labels.to(regression_output.device)
        ...
        if not return_dict:
            output = (regression_output,) # + outputs[2:]
            return ((loss,) + output) if loss is not None else output
...
class RobertaForMaskedLMFast(RobertaPreTrainedModel):
...
# Modified Roberta idea goes here
...
class RobertaTracedEmbeddings(nn.Module):
    """
    RobertaEmbeddings + trace pos emb
    """
...
class RobertaFastTraceEmbedModel(RobertaFastPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
